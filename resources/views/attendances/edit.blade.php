                            ...
                            @foreach($students as $s)
                                @if($s->id == $user_id)
                                    <tr>
                                        ...
                                        <td>{{$s->name}}</td>
                                        <td>{{$s->surname}}</td>
                                        {!!Form::open(['action' => ['AttendancesController@update', $attendance->id], 'method'=>'POST'])!!}
                                            {!!Form::hidden('user_id', $user_id)!!}
                                            {!!Form::hidden('lesson_id', $lesson->id)!!}
                                            <td>{!!Form::text('notes', $attendance->notes,['class' => 'form-control', 'placeholder' => 'Notes']) !!}</td>
                                            <td>{!!Form::checkbox('value', '1',  $attendance->value == 1 ? true : false ,['class' => 'custom-control']) !!}</td>
